...
export class Router {
   ...
   *
   * @param {string} options.root - A CSS selector for the root Element in which
   * all views will be injected.
   *
   * @param {Array} options.routes - An array of route objects. Routes will be
   * matched in the order they appear in the array.
   *
   * @param {Array} options.langs - An array of lanauge slugs.
   *
   * @param {string} options.defaultLang
   *
   * @param {string} [options.cacheViews] - Enable or disable the caching of
   * views. All view loads will be hard reloads. Defaults to `false`.
   *
   * @param {object} [options.models] - A key:value object of models, with keys
   * being the model name, and the values being the location of the `.js` file.
   *
   * @param {number} [options.scrollTopDelay] - When loading a cached view, this
   * is the number of ms to wait before attempting to scroll back to the
   * position that the user left off at. Defaults to 0, which is still one event
   * loop tick.
   *
   * @param {number} [options.maxHistoryEntries] - Max number of items in the
   * history. Defaults to 50.
   *
   * @param {Function} [options.templateGetter] - Optional pluggable function
   * that receives view template file location (string), and returns its raw
   * contents. File contents are cached, so this function will only ever get
   * called once per route load per session.
   * 
   * @param {Function} [options.templateProcessor] - Optional pluggable function
   * that receives the raw template HTML and processes it. Use this function to
   * run the template through ejs, pug, handlebars, or whatever else. Does not
  ...
  constructor(options) {
    ...
    this.templateGetter = 'templateGetter' in options ? options.templateGetter : null
    this.templateProcessor = 'templateProcessor' in options ? options.templateProcessor : null

    this.langs = options.langs
    this.defaultLang = options.defaultLang
    this.currentLang = options.currentLang || options.defaultLang
    this.__ = '__' in options ? options.__ : null

    this._viewCache = {}
    this._history = []
    this.maxHistoryEntries = 'maxHistoryEntries' in options ? options.maxHistoryEntries : 50

    // when a view is loaded from the cache, wait (in ms) before attempting to scroll to the previous scrollTop
    this.scrollTopDelay = 'scrollTopDelay' in options ? options.scrollTopDelay : 0
    ...
    return new Promise(async (resolve, reject) => {
      ...
      if (this.cacheViews && this.currentHref !== null) {
        ...
        // the cache attempt was not blocked
          ...
          this._viewCache[this.currentHref] = document.querySelector(this.root).innerHTML
        }
      }
      ...
      if (this.cacheViews && href in this._viewCache && !hardRefresh) {
        //console.log('%cLoading view from cache', `color:${this.consoleColor};`)
      ...
      // else load the view from the html file
        ...
        //console.log('%cLoading view from file', `color:${this.consoleColor};`)

        viewHtml = this.templateGetter(`/views/${viewFile}`)
        let modelData = null
        ...
        // if the model returned data, make the data available to the view while parsing the view template
        if (modelData) {
          viewHtml = await this.templateProcessor(viewHtml, modelData)
        } else {
          viewHtml = await this.templateProcessor(viewHtml)
        }
      ...
      document.querySelector(this.root).setAttribute('data-current-route', route)
  ...
  _setRouterLinkActiveStates() {
    ...
    if (parentChain.length) {
      for (let routeWithoutLang of parentChain) {
        document.querySelector(`${this.buttonSelector}[href="${routeWithoutLang}"]`).classList.add('active')
      }
    }
  }

  /**
   * Adds a href to the internal history and ensures that the array does not exceed this.maxHistoryEntries.
   * 
  ...
  _addToHistory(href) {
    ...
    if (this._history.length + 1 > this.maxHistoryEntries) {
      this._history = this._history.slice(-Math.abs(this.maxHistoryEntries))
    }
  }
